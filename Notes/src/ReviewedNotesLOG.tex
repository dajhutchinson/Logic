...
\definition{Characteristic Function, $\chi_R$}
...
\remarkk{The meaning of a formulae depends on how we interpret their free variables}
...
\definition{Semantic Value of $\LL$-Terms, $\bar{s}$}
Let $\LL$ be a FOL and $\M:=(|\M|,\I)$ be an $\LL$-Structure.\\
...
\proposition{$\Phi$ is Consistent iff $\exists\ \psi\in Fml_\LL$ st $\Phi\nvdash\psi$}

\proposition{$\Phi$ is Consistent iff $\forall\ \Sigma\subset\Phi$, $\Sigma$ is Consistent}

\proposition{If $\Gamma\vdash\phi$ then $\exists\ \Sigma\subset\Gamma$ st $\Sigma\vdash\phi$.}

\theorem{Inconsistency}
Let $\LL$ be a FOL, $\phi\in Fml_\LL$ and $\Phi\subset Fml_\LL$.
\begin{enumerate}
...
\theorem{If $\Gamma\cup\{\neg\phi\}$ is Satisfiable then $\Gamma\nvdash\phi$.}

\theorem{Chain of Consistency}
...
\definition{Maximally Consistent}
...
\proposition{Let $\Gamma\subset\Sigma\subset Fml_\LL$ if $\Gamma$ is Henkin then $\Sigma$ is Henkin.}
...
\proposition{If $\Gamma\subset Fml_\LL$ is \textit{Consistent} and $\Gamma\vdash\phi\implies\Gamma\cup\{\phi\}$ is \textit{Consistent}.}

\proposition{If $\Gamma\subset Fml_\LL$ is \textit{Consistent} then $\Gamma$ is \textit{Maximally Consistent} \underline{iff} $\psi\in\Gamma$ \underline{or} $\neg\psi\in\Gamma$ holds $\forall\ \psi\in Fml_\LL$.}

\proposition{If $\Gamma$ is \textit{Maximally Consistent} then $\Gamma\vdash\phi$ \underline{iff} $\phi\in\Gamma$.}
...
\proposition{Properties Maximally Consistent Henkin Sets}
...
\propositionn{$\mathcal{R}$ is defined by $\phi$ in $\M$ iff $\M,s\vDash\phi\Leftrightarrow\langle s(x_1),\dots,s(x_k)\rangle\in\mathcal{R}$ for all variable assignments $s$ over $\M$.}

\section{Deductive Reasoning}

\remark{Structure of Deductive Mathematical Proofs}
...
\remark{Logical Axioms are assumptions in almost all mathemtical proofs}
...
\definition{Logical Axioms of Hilbert Calculus, $\Lambda_\LL$}
...
\remark{Logical Axioms are formulae}

\definition{Deduction in Hilbert Calculus}
...
\proposition{Deductions from Deductions}
...
\definition{Deducibility in Hilbert Style}
...
\remark{If $\Gamma\vdash\phi$ we say $\phi$ is a \underline{Theorem} of $\Gamma$}
...
\theorem{Monotonicity of Deducibility}
...
\proposition{Alternative Expressions of prev theorems}
Let $\LL$ be a FOL, $\phi,\psi\in Fml_\LL$ \& $\Gamma,\Sigma\subset Fml_\LL$.
\begin{itemize}
...
\theorem{Law of Excluded Middle - $\forall\ \phi\in Fml_\LL,\ \vdash\phi\vee\neg\phi$}
Since $(\phi\vee\neg\phi)\Leftrightarrow(\neg\phi\to\neg\phi)$.\\

\theorem{Deduction Theorem}
Let $\LL$ be a FOL, $\phi,\psi\in Fml_\LL$ \& $\Gamma,\Sigma\subset Fml_\LL$.
\begin{center}$\Gamma,\phi\vdash\psi$ iff $\Gamma\vdash(\phi\to\psi)$.\end{center}

\theorem{Transitivity of Conditional}
...
\theorem{Right $\forall$ Elimination}
Let $\LL$ be a FOL, $\Gamma\subset Fml_\LL$, $t\in Tm_\LL$ and $\phi,\psi\in Fml_\LL$.
\begin{center}
...
\proposition{$\forall\ \phi,\psi\in Fml_\LL,\ \vdash((\phi\to\psi)\to(\neg\psi\to\neg\phi))$}
...
\theorem{Left $\wedge$ Introduction}
...
\proposition{If $\mathtt{t,x,\phi}$ then $\vdash([\psi]\frac{t}x\to\exists x\phi)$}

\proposition{If $x\not\in FV(\phi)$ then $\vdash(\exists x\phi\to\phi)$}

\proposition{$\vdash(\forall x(\phi\to\psi)\to(\exists x\phi\to\exists x\psi))$}

\proposition{$\vdash (t_0\equiv t_1)\to(t_1\equiv t_0)$}

\proposition{$\vdash (t_0\equiv t_1)\leftrightarrow(t_1\equiv t_0)$}

\proposition{$\vdash (t_0\equiv t_1)\to((t_1\equiv t_2)\to(t_0\equiv t_2))$}

\propositionn{$\{\langle t_0,t_1\rangle \in Tm_\LL\times Tm_\LL : \Gamma\vdash(t_0\equiv t_1)\}$ is an Equivalence Relation.}

\subsection{Completeness \& Soundness Theorems}

\theoremm{Every Logical Axiom is logically true}
Let $\LL$ be a FOL.
\begin{center}$\forall\ \lambda\in\Lambda_\LL,\ \vDash \lambda$ (\ie $\emptyset\vDash\lambda$)\end{center}

\theoremm{Soundess Theorem}
Let $\LL$ be a FOL.
...
\remark{The Completeness Theorem makes Hilbert Calculus unnecessary.}
Proving $(\vdash\phi)\leftrightarrow(\neg\neg\phi)$ is not obvious but $(\vDash\phi)\leftrightarrow(\neg\neg\phi)$ is obvious.\\

\theorem{Restricted Completeness Theorem}
Let $\LL$ be a FOL and $\Gamma\subset Fml_\LL$.\\
...
\remark{Combining Soundness \& Completeness Theorem}
Let $\LL$ be a FOL.\\
...
\definition{Eqivalence Relation $\sim_\Gamma$}
...
\proposition{Equivalence Relation for Functions and Predicates}
...
\definition{Canonical Model $\M^\Gamma$ and Canonical Variable Assignment $s^\Gamma$}
...
\remark{$|\M^\Gamma|$ is Countable}

\proposition{Properties of Canonical Model}
...
\definition{Consequences of $\Gamma$}
...
\begin{tabular}{ccccc}
	...
	\begin{tabular}{|cc|c|c|c|c|}
		\hline
		$P$&$Q$&$P\to Q$&$P\leftrightarrow Q$&$P\wedge Q$&$P\vee Q$\\\hline
		T&T&\textbf{T}&\textbf{T}&\textbf{T}&\textbf{T}\\
		T&F&\textbf{F}&\textbf{F}&\textbf{F}&\textbf{T}\\
		F&T&\textbf{T}&\textbf{F}&\textbf{F}&\textbf{T}\\
		F&F&\textbf{T}&\textbf{T}&\textbf{F}&\textbf{F}\\\hline
	\end{tabular}&
